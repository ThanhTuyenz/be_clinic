...
export class MedicalVisitsService {
  ...
  private serialize(row: any) {
    ...
    if (row.vitals) {
      ...
      payload.temperature = vitals.temperature === null ? '' : Number(vitals.temperature)
      payload.respiratoryRate = vitals.respiratoryRate ?? ''
      payload.bp = vitals.systolicBp && vitals.diastolicBp ? `${vitals.systolicBp}/${vitals.diastolicBp}` : ''
      payload.pulse = vitals.pulse ?? ''
      payload.heightCm = vitals.heightCm === null ? '' : Number(vitals.heightCm)
      payload.weightKg = vitals.weightKg === null ? '' : Number(vitals.weightKg)
      payload.spo2 = vitals.spo2 === null ? '' : Number(vitals.spo2)
    }
    if (row.prescription) {
  ...
  async upsert(userId: string, body: Record<string, unknown>) {
    ...
    const row = await this.prisma.$transaction(async (tx) => {
      ...
      if (Object.values(vitalData).some((value) => value !== null)) {
        await tx.visitVitals.upsert({
          where: { medicalVisitId: visit.id },
          create: { medicalVisitId: visit.id, ...vitalData },
          update: vitalData,
        })
      } else {
        await tx.visitVitals.deleteMany({ where: { medicalVisitId: visit.id } })
      }
