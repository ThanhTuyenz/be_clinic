...
export function grantedMatchRequired(
  ...
  const matchingPermissions = grantedPermissions.filter(
    (p) =>
      (p.resourceType === requiredPermission.resourceType ||
        p.resourceType === PermissionResource.ANY) &&
      (p.action === requiredPermission.action ||
        p.action === PermissionAction.ANY) &&
...
export function getPermittedResourcesIds(
  ...
  return grantedPermissions
    .filter(
      (p) =>
        p.effect === effect &&
        [PermissionResource.ANY, requiredPermission.resourceType].includes(
          p.resourceType,
        ) &&
