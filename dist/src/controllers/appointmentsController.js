...
exports.getDoctorScheduleDates = getDoctorScheduleDates;
exports.lookupAppointmentByTicket = lookupAppointmentByTicket;
exports.createAppointmentReception = createAppointmentReception;
exports.lookupPatientByCode = lookupPatientByCode;
exports.listPatientsReception = listPatientsReception;
exports.listPatientHistoryReception = listPatientHistoryReception;
exports.listReceptionAppointments = listReceptionAppointments;
exports.updateAppointmentStatusReception = updateAppointmentStatusReception;
const mongoose_1 = __importDefault(require("mongoose"));
const bcryptjs_1 = __importDefault(require("bcryptjs"));
const Appointment_js_1 = __importDefault(require("../models/Appointment.js"));
...
const scheduleAvailability_js_1 = require("../services/scheduleAvailability.js");
...
function queueAppointmentConfirmationEmail({ patient, doctor, appointment, ticket }) {
    ...
    if (!to || !to.includes('@'))
        return;
    void (0, mail_js_1.sendAppointmentConfirmationEmail)({
        to,
...
async function findUserByIdFlexible(userId, projection) {
    ...
    if (!id)
        return null;
    let user = await User_js_1.default.collection.findOne({ _id: id }, { projection });
    if (!user && isMongoObjectId(id)) {
        user = await User_js_1.default.collection.findOne({ _id: new mongoose_1.default.Types.ObjectId(id) }, { projection });
    }
    return user;
}
...
function shouldAutoAssignVisitQueueNumber(body, prevStatus) {
    if (hasExplicitVisitQueueNumberInBody(body))
        return false;
...
async function getNextVisitQueueNumberForRoom({ appointmentDate, clinicRoom, excludeAppointmentId }) {
    ...
    if (ex && isMongoObjectId(ex)) {
        query._id = { $ne: new mongoose_1.default.Types.ObjectId(ex) };
    }
    const top = await Appointment_js_1.default.findOne(query)
...
async function listMyAppointments(req, res) {
    try {
        ...
        const items = await Appointment_js_1.default.find({
        ...
        const doctors = doctorIdsRaw.length > 0
            ? await User_js_1.default.collection
                .find({
                userType: 'doctor',
                $or: [
                    { _id: { $in: doctorIdsRaw } },
        ...
        const specialtyIds = Array.from(new Set(doctorDocs
            .map((d) => d?.specialtyID ?? d?.specialtyId)
            .filter(Boolean)
            .map((id) => String(id).trim())));
        const specialties = specialtyIds.length
            ? await Specialty_js_1.default.find({ specialtyID: { $in: specialtyIds } }, { specialtyID: 1, specialtyName: 1, deptID: 1 }).lean()
            : [];
        const specialtyNameById = new Map(specialties.map((s) => [String(s.specialtyID), String(s.specialtyName || '').trim()]));
        ...
        const departments = deptIds.length
            ...
            : [];
        const deptNameById = new Map(departments.map((d) => [String(d.deptID), String(d.deptName || '').trim()]));
        let didLog = false;
        ...
        return res.status(200).json({
            appointments: (items || []).map((a) => {
                ...
                const rawSpecialtyName = String(doc.specialtyName || doc.specialty || doc.specialization || '').trim();
                const rawDeptName = String(doc.deptName || doc.dept || doc.departmentName || '').trim();
                const specialtyName = specialtyID ? specialtyNameById.get(specialtyID) || rawSpecialtyName : rawSpecialtyName;
...
async function listDoctorAppointments(req, res) {
    try {
        ...
        const items = await Appointment_js_1.default.find({ doctorId: doctorIdStr })
            .sort({ appointmentDate: -1, startTime: 1, createdAt: -1 })
            .lean();
        ...
        const patientObjectIds = patientIdsRaw
            .filter(isMongoObjectId)
            .map((id) => new mongoose_1.default.Types.ObjectId(id));
        const patients = patientIdsRaw.length > 0
            ? await User_js_1.default.collection
                .find({
                userType: 'patient',
                $or: [
...
async function createAppointment(req, res) {
    try {
        ...
        let doctor = await User_js_1.default.collection.findOne({ _id: doctorIdStr });
        if (!doctor && isMongoObjectId(doctorIdStr)) {
            doctor = await User_js_1.default.collection.findOne({ _id: new mongoose_1.default.Types.ObjectId(doctorIdStr) });
        }
        if (!doctor) {
            return res.status(400).json({
                message: `Không có user với id="${doctorIdStr}" trong MongoDB mà backend đang kết nối (db="${mongoose_1.default.connection.name}"). ` +
        ...
        const patientTimeConflict = await Appointment_js_1.default.findOne({
            patientId: req.user.id,
        ...
        const patientDoctorConflict = await Appointment_js_1.default.findOne({
            patientId: req.user.id,
            doctorId: doctorIdStr,
            status: { $in: ACTIVE_STATUSES },
        })
        ...
        if (patientDoctorConflict) {
            return res.status(409).json({
                message: 'Bạn đã có một lịch khám đang chờ/xác nhận với bác sĩ này. ' +
                    'Chỉ đặt lại sau khi lịch đó hoàn thành hoặc đã hủy.',
            });
        }
        let appointment;
        try {
            appointment = await Appointment_js_1.default.create({
                patientId: req.user.id,
                doctorId: doctorIdStr,
                appointmentDate: date,
                startTime: startTimeStr,
        ...
        const patient = await User_js_1.default.findById(req.user.id).lean();
...
async function cancelAppointment(req, res) {
    try {
        ...
        const appt = await Appointment_js_1.default.findById(rawId);
        if (!appt) {
        ...
        const reasonRaw = req.body && typeof req.body === 'object'
...
async function getAvailability(req, res) {
    try {
        ...
        const rows = await Appointment_js_1.default.find({
            doctorId,
            appointmentDate: date,
            status: { $in: ['pending', 'confirmed'] },
        }, { startTime: 1, _id: 0 }).lean();
        const bookedStartTimes = Array.from(new Set((rows || []).map((r) => String(r.startTime || '').trim()).filter(Boolean))).sort();
        const bookedSet = new Set(bookedStartTimes);
        const db = mongoose_1.default.connection.db;
        const { slots, shifts, hasSchedule } = await (0, scheduleAvailability_js_1.computeAvailabilityFromSchedule)({
            db,
            doctorId,
            dateStr,
            bookedSet,
...
async function getDoctorScheduleDates(req, res) {
    try {
        const userType = String(req.user?.userType || '').toLowerCase();
        const doctorId = String(req.query.doctorId || '').trim();
        ...
        const from = fromStr && (0, scheduleAvailability_js_1.isValidIsoDateOnly)(fromStr) ? fromStr : defaultFrom;
        const to = toStr && (0, scheduleAvailability_js_1.isValidIsoDateOnly)(toStr) ? toStr : defaultTo;
        const dates = await (0, scheduleAvailability_js_1.findDoctorScheduleDateKeys)(mongoose_1.default.connection.db, doctorId, from, to);
...
async function findPatientUserByContact(rawLogin) {
    ...
    if (!raw)
        return null;
    if (raw.includes('@')) {
        return User_js_1.default.findOne({ userType: 'patient', email: raw.toLowerCase() }).lean();
    }
    return User_js_1.default.findOne({ userType: 'patient', phone: raw }).lean();
}
...
function parseGenderToBooleanOrNull(value) {
    ...
    if (!s)
        return null;
    if (s === 'nam' || s === 'male' || s === 'm' || s === 'true')
        return true;
    if (s === 'nữ' || s === 'nu' || s === 'female' || s === 'f' || s === 'false')
        return false;
    return null;
}
async function createPatientForReception({ patientInfo, patientEmailOrPhone }) {
    ...
    const existing = await User_js_1.default.findOne({
        $or: [{ email }, { phone }],
    }).lean();
    if (existing) {
        return {
    ...
    const role = await Role_js_1.default.findOne({ name: 'patient' }).lean();
    if (!role) {
        return {
            errorStatus: 500,
    ...
    const passwordHash = await bcryptjs_1.default.hash('111111', 10);
    const created = await User_js_1.default.create({
        email,
        passwordHash,
...
async function lookupAppointmentByTicket(req, res) {
    try {
        ...
        const candidates = await Appointment_js_1.default.find({
            appointmentDate: { $gte: dayStart, $lte: dayEnd },
            status: { $in: ['pending', 'confirmed', 'cancelled'] },
        }).lean();
        const want = ticketRaw.toUpperCase().replace(/\s+/g, '');
        const matches = (candidates || []).filter((a) => buildTicketCode(a._id, a.appointmentDate).toUpperCase() === want);
        if (!matches.length) {
        ...
        const patientObjectIds = isMongoObjectId(pid) ? [new mongoose_1.default.Types.ObjectId(pid)] : [];
        const patients = patientIdsRaw.length > 0
            ? await User_js_1.default.collection
                .find({
                userType: 'patient',
        ...
        const doctorObjectIds = isMongoObjectId(did) ? [new mongoose_1.default.Types.ObjectId(did)] : [];
        const doctors = doctorIdsRaw.length > 0
            ? await User_js_1.default.collection
                .find({
                userType: 'doctor',
        ...
        if (specialtyId) {
            const sp = await Specialty_js_1.default.findOne({ specialtyID: String(specialtyId).trim() }, { specialtyName: 1 }).lean();
        ...
        const doctorRoomMetaMap = doctorRoomId ? await (0, clinicRoomHelper_js_1.getClinicRoomMetaMap)([doctorRoomId]) : new Map();
        ...
        return res.status(200).json({
            ...
            doctor: doc
                ? {
                    ...
                    clinicRoomName: doctorRoomMeta ? (0, clinicRoomHelper_js_1.clinicRoomDisplayLabel)(doctorRoomId, doctorRoomMeta) : doctorRoomId,
                    consultationFee: resolveConsultationFee(doc.consultationFee),
                }
                : null,
            consultationFee: doc ? resolveConsultationFee(doc.consultationFee) : consultationFee_js_1.DEFAULT_CONSULTATION_FEE,
        });
    }
    catch (err) {
        console.error(err);
...
async function createAppointmentReception(req, res) {
    try {
        ...
        if (patientDoctorConflict) {
            return res.status(409).json({
                message: 'Bệnh nhân đã có lịch đang chờ/xác nhận với bác sĩ này. Chỉ đặt lại sau khi lịch đó hoàn thành hoặc đã hủy.',
            });
        }
        let appointment;
        try {
...
async function lookupPatientByCode(req, res) {
    try {
        if (String(req.user?.userType || '') !== 'receptionist' && String(req.user?.userType || '') !== 'registration') {
        ...
        const patients = await User_js_1.default.find({
            userType: 'patient',
            isActive: { $ne: false },
        })
...
async function listPatientsReception(req, res) {
    try {
        if (String(req.user?.userType || '') !== 'receptionist' && String(req.user?.userType || '') !== 'registration') {
        ...
        const pageSize = Number.isFinite(pageSizeRaw) && pageSizeRaw > 0 && pageSizeRaw <= 50 ? Math.floor(pageSizeRaw) : 10;
        ...
        const accountQ = String(req.query.account || '').trim().toLowerCase();
        if (patientCodeQ) {
        ...
        if (phoneQ)
            query.phone = { $regex: escapeRegex(phoneQ), $options: 'i' };
        if (accountQ)
            query.email = { $regex: escapeRegex(accountQ), $options: 'i' };
        const projection = {
            firstName: 1,
            lastName: 1,
            email: 1,
        ...
        const raw = await User_js_1.default.find(query).select(projection).sort({ createdAt: -1 }).lean();
        let rows = (raw || []).map((u) => ({
            id: u._id,
...
async function listPatientHistoryReception(req, res) {
    try {
        if (String(req.user?.userType || '') !== 'receptionist' && String(req.user?.userType || '') !== 'registration') {
        ...
        const items = await Appointment_js_1.default.find({ patientId })
            .sort({ appointmentDate: -1, startTime: -1, createdAt: -1 })
            .limit(50)
            .lean();
        const doctorIdsRaw = [...new Set((items || []).map((a) => String(a?.doctorId || '').trim()).filter(Boolean))];
        const doctorObjectIds = doctorIdsRaw.filter(isMongoObjectId).map((id) => new mongoose_1.default.Types.ObjectId(id));
        const doctors = doctorIdsRaw.length > 0
            ? await User_js_1.default.collection
                .find({
                userType: 'doctor',
        ...
        const specialtyIds = Array.from(new Set(doctors
            ...
            .map((id) => String(id).trim())));
        const specialties = specialtyIds.length
            ? await Specialty_js_1.default.find({ specialtyID: { $in: specialtyIds } }, { specialtyID: 1, specialtyName: 1 }).lean()
            : [];
        const specialtyNameById = new Map(specialties.map((s) => [String(s.specialtyID), String(s.specialtyName || '').trim()]));
        return res.status(200).json({
            appointments: (items || []).map((a) => {
...
async function listReceptionAppointments(req, res) {
    try {
        ...
        const items = await Appointment_js_1.default.find(query)
            .sort({ appointmentDate: 1, startTime: 1, createdAt: 1 })
            .lean();
        const patientIdsRaw = [...new Set((items || []).map((a) => String(a?.patientId || '').trim()).filter(Boolean))];
        const doctorIdsRaw = [...new Set((items || []).map((a) => String(a?.doctorId || '').trim()).filter(Boolean))];
        const patientObjectIds = patientIdsRaw.filter(isMongoObjectId).map((id) => new mongoose_1.default.Types.ObjectId(id));
        const doctorObjectIds = doctorIdsRaw.filter(isMongoObjectId).map((id) => new mongoose_1.default.Types.ObjectId(id));
        const patients = patientIdsRaw.length > 0
            ? await User_js_1.default.collection
                .find({
        ...
        const doctorRoomMetaMap = await (0, clinicRoomHelper_js_1.getClinicRoomMetaMap)(doctorRoomIdsForList);
                ...
                doctor: doc
                    ? {
                        ...
                        clinicRoomName: (() => {
                            ...
                            return m ? (0, clinicRoomHelper_js_1.clinicRoomDisplayLabel)(rid, m) : rid;
                        })(),
                        consultationFee: resolveConsultationFee(doc.consultationFee),
                    }
                    : null,
...
async function updateAppointmentStatusReception(req, res) {
    try {
        ...
        if (status === 'cancelled') {
            ...
            const bySystem = req.body?.cancelledBySystem === true ||
                req.body?.cancelledBySystem === 'true' ||
                req.body?.cancelledBySystem === 1;
            if (bySystem) {
                if (!isPendingAppointmentPastSlotServer(appt)) {
                    return res.status(400).json({
                        message: 'Không thể ghi nhận hủy tự động: lịch chưa qua khung giờ hoặc không còn ở trạng thái chờ.',
                    });
                }
                appt.cancelReason =
