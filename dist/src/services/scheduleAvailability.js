...
exports.DEFAULT_SLOT_MINUTES = void 0;
exports.isValidIsoDateOnly = isValidIsoDateOnly;
exports.isValidHHmm = isValidHHmm;
exports.buildSlotTimesFromRange = buildSlotTimesFromRange;
exports.findDoctorSchedulesForDate = findDoctorSchedulesForDate;
exports.findDoctorScheduleDateKeys = findDoctorScheduleDateKeys;
exports.loadShiftMap = loadShiftMap;
exports.computeAvailabilityFromSchedule = computeAvailabilityFromSchedule;
exports.DEFAULT_SLOT_MINUTES = 12;
...
function buildSlotTimesFromRange(startTime, endTime, slotMinutes = exports.DEFAULT_SLOT_MINUTES) {
...
async function findDoctorSchedulesForDate(db, doctorId, dateStr) {
...
async function findDoctorScheduleDateKeys(db, doctorId, fromStr, toStr) {
    const coll = db.collection('doctorSchedule');
    ...
    for (const row of rows) {
        ...
        if (k && k >= fromStr && k <= toStr)
            keys.add(k);
    }
    return Array.from(keys).sort();
}
async function loadShiftMap(db, shiftIds) {
...
async function computeAvailabilityFromSchedule({ db, doctorId, dateStr, bookedSet, now = new Date() }) {
    const schedules = await findDoctorSchedulesForDate(db, doctorId, dateStr);
    if (!schedules.length) {
        return { slots: [], shifts: [], hasSchedule: false };
    }
    const shiftMap = await loadShiftMap(db, schedules.map((s) => s.shiftID));
    ...
    for (const sched of schedules) {
        ...
        const slotMinutes = Number(shift.slotMinutes) > 0 ? Number(shift.slotMinutes) : exports.DEFAULT_SLOT_MINUTES;
        ...
        for (const t of times) {
            if (bookedSet.has(t))
                continue;
            if (isToday && hhmmToMinutes(t) <= nowMinutes)
                continue;
            slotSet.add(t);
            freeInShift.push(t);
        }
...
//# sourceMappingURL=scheduleAvailability.js.map